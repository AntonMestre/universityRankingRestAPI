<?php
	...
	$nbrUniversity = $_GET["nbrUniversity"];
	$population = $_GET["population"];
	$inhabitantsByUniversities=$population/$nbrUniversity;
	$area=$_GET["area"];
	$areaByUniversity=$area/$nbrUniversity;

	$link = mysqli_connect($PARAM_hote,$PARAM_user,$PARAM_pw,$PARAM_bdd);
	
	$query="SELECT name,numberOfUniversities FROM rankingCountry WHERE name = '".$_GET["countryName"]."'";
	$r_query = $link->query($query);
	
	if (mysqli_num_rows($r_query)==0) {
		$sql = "INSERT INTO rankingCountry VALUES (NULL,'".$_GET['countryName']."', ".$nbrUniversity.", ".$inhabitantsByUniversities.",".$areaByUniversity.")";
		$link->query($sql);
	}

    $sql = "UPDATE rankingCountry SET numberOfUniversities=".$nbrUniversity.",InhabitantsByUniversities=".$inhabitantsByUniversities.", AreaByUniversity=".$areaByUniversity." WHERE name = '".$_GET["countryName"]."'";
    $link->query($sql);
