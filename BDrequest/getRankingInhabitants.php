<?php
	...
	$query = "SELECT name,inhabitantsByUniversities FROM rankingCountry ORDER BY inhabitantsByUniversities ASC LIMIT 10";

	$arr = array();
	$r_query = $link->query($query);
	
	while($row = $r_query->fetch_assoc()) {
		$arr[$row["name"]]=$row["inhabitantsByUniversities"];
	}
